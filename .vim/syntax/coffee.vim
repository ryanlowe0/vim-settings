...
syn match coffeeKeyword /\<\%(\|class\|do\)\>/  display
...
syn match coffeeOperator /\<\%(new\|in\|of\|by\|and\|or\|not\|is\|isnt\|extends\|super\)\>/
\                       display
...
hi def link coffeeBoolean Structure

syn match coffeeGlobal /\<\%(null\|undefined\)\>/ display
hi def link coffeeGlobal Structure
...
hi def link coffeeSpecialIdent Function

syn match coffeeJQfollow /\%((\|[ .,]\)/

" JQuery $
" syn match coffeeJQuery /\$\%(\I\i*\)\?/ display
syn match coffeeJQuery /\$/ nextgroup=coffeeJQfollow display
hi def link coffeeJQuery Type

" A class-like name that starts with a capital letter
syn match coffeeObject /\<\u[A-Z][a-z0-9_]\+\>/ display
hi def link coffeeObject Special

" A constant-like name in SCREAMING_CAPS
syn match coffeeConstant /\<\u[A-Z][A-Z0-9_]\+\>/ display
hi def link coffeeConstant Structure
...
\                       contains=@coffeeAll
hi def link coffeeInterpDelim Special

" A string escape sequence
syn match coffeeEscape /\\\d\d\d\|\\x\x\{2\}\|\\u\x\{4\}\|\\./ contained display
hi def link coffeeEscape Special
